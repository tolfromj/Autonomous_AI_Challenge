...
def main(model_filename, input_test_path, output_path):
    ...
    for index, img_filename in enumerate(img_files):
        if 8652 > index or index > 8653:
            continue

        result = model.predict(img_filename, imgsz=1280, conf=0.001, iou=0.6, augment=False)[0]
