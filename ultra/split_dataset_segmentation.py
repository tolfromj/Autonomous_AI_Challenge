...
def split_dataset(directory):
    ...
    train_labels_path = os.path.join(directory,'labels/train')
    train_images_path = os.path.join(directory,'images/train')
    ...
    if len(train_files) + len(val_files) != total_files:
        raise ValueError("The total number of training and validation data is incorrect.")
    
    # val 디렉토리 만들기.
    val_labels_path = os.path.join(directory, 'labels/val')
    val_images_path = os.path.join(directory, 'images/val')
    ...
    for val_file in tqdm(val_files):
        ...
        source_images_path = os.path.join(train_images_path, f'{val_file}.png')
        destination_labels_path = os.path.join(val_labels_path, f'{val_file}.txt')
        destination_images_path = os.path.join(val_images_path, f'{val_file}.png')
    ...
    print(f"Dataset split completed. {len(val_files)} files in val and {len(train_files)} files in train")

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--data_path", type=str, default="../data/segmentation_coco"   
    )
