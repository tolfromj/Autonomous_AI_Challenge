...
def split_dataset(directory):
    ...
    train_labels_dir = os.path.join(directory, "labels/train")
    train_images_dir = os.path.join(directory, "images/train")
    ...
    for i in range(3):
        ...
        # val 디렉토리 만들기.
        val_labels_path = os.path.join(directory, f"labels/val{i}")
        val_images_path = os.path.join(directory, f"images/val{i}")
        train_labels_path = os.path.join(directory, f"labels/train{i}")
        train_images_path = os.path.join(directory, f"images/train{i}")
        ...
        for train_file in tqdm(train_files):
            ...
            source_images_path = os.path.join(train_images_dir, f"{train_file}.png")
            # 복사될 위치의 train path
            destination_train_labels_path = os.path.join(
                directory, f"labels/train{i}/{train_file}.txt"
            )
            destination_train_images_path = os.path.join(
                directory, f"images/train{i}/{train_file}.png"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="/workspace/traffic_light/data/segmentation_coco",
    )
