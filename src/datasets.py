...
class TrafficLightDataset(Dataset):
    ...
    def _load_bbox(self, id, orig_width, orig_height):
        ...
        for anno_id in self.coco.getAnnIds(id):
            ...
            bboxes.append(self._coco_to_pascal_bbox(id, anns["bbox"], orig_width, orig_height))
            area.append(anns["area"])
        return bboxes, labels, area

    def _coco_to_pascal_bbox(self, id, anns, orig_width, orig_height):
        x_min = anns[0]
        x_max = anns[0] + anns[2]
        y_min = anns[1]
        y_max = anns[1] + anns[3]
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, dict]:
        ...
        bboxes, labels, area = self._load_bbox(index, orig_width, orig_height)  # type of these are python
        ...
        # labels = self.formatted_anns(index, labels, area, bboxes, orig_width, orig_height)
        ...
        # target["orig_size"] = torch.tensor([orig_width, orig_height], dtype=torch.int64)
